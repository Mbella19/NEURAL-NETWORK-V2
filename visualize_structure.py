...
def visualize_structure():
    ...
    # Plot BOS/CHoCH
    # These happen at the BREAK candle (current time)
    bos_bull_idx = np.where(df_slice.bos_bullish)[0]
    bos_bear_idx = np.where(df_slice.bos_bearish)[0]
    choch_bull_idx = np.where(df_slice.choch_bullish)[0]
    choch_bear_idx = np.where(df_slice.choch_bearish)[0]
    
    for idx in bos_bull_idx:
        ax.axvline(x=idx, color='green', linestyle='--', alpha=0.5)
        ax.text(idx, df_slice.high.iloc[idx] + 0.0005, 'BOS', color='green', rotation=90)
        
    for idx in bos_bear_idx:
        ax.axvline(x=idx, color='red', linestyle='--', alpha=0.5)
        ax.text(idx, df_slice.low.iloc[idx] - 0.0005, 'BOS', color='red', rotation=90)
        
    for idx in choch_bull_idx:
        ax.axvline(x=idx, color='blue', linestyle='-', alpha=0.7)
        ax.text(idx, df_slice.high.iloc[idx] + 0.0005, 'CHoCH', color='blue', rotation=90)
        
    for idx in choch_bear_idx:
        ax.axvline(x=idx, color='orange', linestyle='-', alpha=0.7)
        ax.text(idx, df_slice.low.iloc[idx] - 0.0005, 'CHoCH', color='orange', rotation=90)
        
    plt.title('Market Structure Analysis (Fractals, BOS, CHoCH)')
    plt.legend()
